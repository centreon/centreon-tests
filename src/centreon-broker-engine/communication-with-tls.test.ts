...
describe('engine and broker testing in same time for compression', () => {
   ...
   it('tls with keys checks between broker - engine (common name)', async () => {
        ...
        // generates keys
        await shell.exec("openssl req -new -newkey rsa:2048 -days 365 -nodes -x509 -keyout /etc/centreon-broker/server.key -out /etc/centreon-broker/server.crt -subj '/CN=centreon'")
        await shell.exec("openssl req -new -newkey rsa:2048 -days 365 -nodes -x509 -keyout /etc/centreon-broker/client.key -out /etc/centreon-broker/client.crt -subj '/CN=centreon'")
        ...
        expect(await Broker.checkLogFileContains(["[tls] [info] TLS: common name 'centreon' used for certificate verification"])).toBeTruthy()
